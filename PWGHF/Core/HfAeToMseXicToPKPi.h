...
namespace o2::analysis
{
	...
	class HfAeToMseXicToPKPi : public HfMlResponse<TypeOutputScore>
	{
	  public:
  			...
  			// Open the file for reading
    			...
    			// Close the file
    				...
    				if (d.HasParseError()) {
	    				...
	    				LOG(fatal) << "Error at offset: " << d.GetErrorOffset() << "\n";
				} else {
		...
		std::vector<float> getScalePars(int pTBin, std::vector<std::vector<float>> scaleParsMatrix){
			if( pTBin >= int(scaleParsMatrix.size()) || pTBin < 0) {
				LOG(fatal) << "No JSON for ML pre-processing in PtBins: " << pTBin;
				return {}; 
				}
			else {
				//LOG(info) << "PtBins: " << pTBin << "size "<< scaleParsMatrix.at(pTBin).size();
				return scaleParsMatrix.at(pTBin);
				}
		}
